...
//echo $map_atts['id'];
$map_post = get_post( $map_atts['id'] );
$instance = get_post_meta( $map_atts['id'] );

//echo $map_post->post_title;
//print_r($map_post_meta);
...
wp_enqueue_style('sgm-front', plugins_url('sgm_front.css', __FILE__) );
	...
	echo '<div class="map-box" style="height: '.$instance['map_height'][0].'px;">';
		echo '<div id="'.$map_post->post_name.'" class="map-container">';
		echo '</div>';
	echo '</div>';
...
<script type="text/javascript">
	jQuery(document).ready(function() {
		<?php
		if($instance['map_style'][0]) {
			?>
			var map_style = '<?php echo $instance['map_style'][0]; ?>';
			<?php
		} else {
			?>
			var map_style = '';
		...
		if($instance['lattitude'][0]) {
			?>
			var lattitude = <?php echo $instance['lattitude'][0]; ?>;
			<?php
		}
		if($instance['longitude'][0]) {
			?>
			var longitude = <?php echo $instance['longitude'][0]; ?>;
			<?php
		}
	...
	var map_$rand = new google.maps.Map(document.getElementById('<?php echo $map_post->post_name; ?>'), options);
